...
(The setting / overview / appendix files were already rewritten before this initiative; these six powers are the work so far.)
...
## Running-start diagnosis for the last power
...
One remains: the **Union of Frontier Freeholds** — the diversity-of-lives angle in full (libertarian *ideology* vs uneven frontier *reality*: who is actually free; the debt-bound, the company towns, the Cartel's grip; the mercenary-company economy). Heavily Cartel-penetrated (Nexus / Wayline; **Korsen's Anchorage**), so keep it consistent with the Cartel files and the invented **Sefkir Reach** (the Freehold that expelled every Cartel subsidiary in 3829 C). Register precedents now set across four rewrites: the Mandate's *unevenness + autumn*; Vega's *savage honesty + seen/unseen*; the Drift's *hard-won joy + gulf-between-decks*. Recipe unchanged: familiarise (confirm the file list first) → diagnose the hat → agree thesis + fork → draft → consistency → compile → commit.
